...
- `MarqSpec.Client.ProjectX/` — Library implementing REST client, WebSocket client, models and DI helpers.
- `MarqSpec.Client.ProjectX.Samples/` — Console sample(s) demonstrating REST + real-time WebSocket usage.
- `MarqSpec.Client.ProjectX.Diagnostics/` — Diagnostic tools for troubleshooting API contract search and connectivity.
- `MarqSpec.Client.ProjectX.Tests/` — Unit and integration tests.
- `docs/` — Project documentation and summaries (also exposed in Solution Items → Summary).
...
```bash
dotnet build
cd MarqSpec.Client.ProjectX.Tests
dotnet test
```
